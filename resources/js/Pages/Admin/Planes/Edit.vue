...
const form = useForm({
    ...
    monto_sugerido: props.plan.monto_sugerido,
    monto_libre: props.plan.monto_libre,
    frecuencia: props.plan.frecuencia,
    beneficios: Array.isArray(props.plan.beneficios) ? [...props.plan.beneficios] : [],
...
const submit = () => form.put(`/admin/planes/${props.plan.id}`);
</script>

<template>
    <Head title="Editar Plan — AJDUT Mexico" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link href="/admin/planes" class="text-slate-400 hover:text-slate-600">Planes</Link>
                <span class="text-slate-300">/</span>
                <span class="font-bold text-slate-800">Editar Plan</span>
            </div>
        </template>
            ...
            <form @submit.prevent="submit" class="space-y-6 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.icono" type="text" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-teal-400 focus:outline-none" />
                    </div>
                </div>
