...
<template>
    ...
    <GuestLayout>
        ...
        <div v-if="page.props.flash?.success" class="mb-4 rounded-xl bg-teal-50 border border-teal-200 px-4 py-3 text-sm font-medium text-teal-700">
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div>
                ...
                <input v-model="form.telefono" type="tel" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-teal-400 focus:outline-none" />
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1">Asunto *</label>
                <input v-model="form.asunto" type="text" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-teal-400 focus:outline-none" required />
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1">Mensaje *</label>
                <textarea v-model="form.mensaje" rows="4" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:border-teal-400 focus:outline-none" required></textarea>
            </div>
            <button type="submit" :disabled="form.processing"
                class="w-full rounded-xl bg-gradient-to-r from-teal-600 to-emerald-600 px-4 py-3 text-sm font-bold text-white shadow hover:from-teal-700 disabled:opacity-50">
                Enviar mensaje
            </button>
        </form>
    </GuestLayout>
